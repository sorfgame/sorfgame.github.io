...
cc.Pomelo = function() {
  ...
  pomelo.connect = function(params,url,cb){
    connect(params,url,cb);
  }
  var connect = function(params, url, cb) {
    ...
    var onclose = function(event) {
      ...
      console.log("//////onclose",JSON.stringify(event),event.code,event.isTrusted);
      if(!event.isTrusted){
        console.error('socket close: ', event);
      }
      if(!!params.reconnect && reconnectAttempts < maxReconnectAttempts) {
        cc.log("////===>>>>>",params.reconnect,reconnectAttempts,maxReconnectAttempts)
        reconnect = true;
        reconnectAttempts++;
  ...
  pomelo.disconnect = function(cb) {
    ...
    disconnectCb = cb;
    reconnectAttempts = DEFAULT_MAX_RECONNECT_ATTEMPTS;
    if(socket) {
  ...
  pomelo.request = function(route, msg, cb) {
    console.log("/////////request====>"+"route:"+route+",msg:",msg);
